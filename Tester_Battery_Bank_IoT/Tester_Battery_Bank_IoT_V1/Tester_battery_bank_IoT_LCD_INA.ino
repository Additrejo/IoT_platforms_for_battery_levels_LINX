...
void loop() {
    ...
    Serial.printf("V: %.2f V | B: %.1f%% | I: %.3f A | P: %.3f W | T: %.2f°C | Estado: %s | Ciclos: %d\n", 
                  voltage, percentage, corriente, potencia, temperatura, estadoBateria.c_str(), cicloContador);
    
    mostrarDatosLCD(voltage, percentage, temperatura, corriente, potencia, estadoBateria, cicloContador);

    if (millis() - tiempoUltimoEnvio >= intervaloEnvio) {
        sendToThingSpeak(voltage, percentage, temperatura, corriente, potencia, cicloContador);
...
void mostrarDatosLCD(float voltage, float percentage, float temperatura, float corriente, float potencia, String estado, int ciclos) {
    lcd.clear();
    lcd.setCursor(0, 0);
...
void sendToThingSpeak(float voltage, float percentage, float temperatura, float corriente, float potencia, int ciclos) {
