...
void loop() {
    ...
    // 🔄 Control de carga/descarga con conteo de ciclos
    static bool previoEnCarga = false;  // Estado anterior de carga
    ...
    if (enCarga) {
        ...
        if (voltage <= 3.0) {
            ...
            // 🟢 Si antes estaba descargando y ahora empieza a cargar, contar un nuevo ciclo
            if (!previoEnCarga) {
                cicloContador++;
    ...
    Serial.printf("Voltaje: %.2f V | Batería: %.1f%% | Temp: %.2f°C | Estado: %s | Ciclos: %d\n", 
                  voltage, percentage, temperatura, estadoBateria.c_str(), cicloContador);
    displayData(voltage, percentage, temperatura, estadoBateria, cicloContador);
...
void displayData(float voltage, float percentage, float temperatura, String estado, int ciclos) {
    display.clearDisplay();
    ...
    display.printf("Estado: %s | Ciclo: %d", estado.c_str(), ciclos);
    display.display();
}
...
void sendToThingSpeak(float voltage, float percentage, float temperatura, int ciclos) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        String url = String(server) + "/update?api_key=" + apiKey +
                     ...
                     "&field4=" + String(ciclos);  // Enviar el contador de ciclos
